...
class TMXObject extends egret.Sprite {
    ...
    private parseTMXShapes(): Array<any> {
        ...
        if (this._isEllipse) {
            var _ellipse: Ellipse   = new Ellipse(0, 0, this.width, this.height);
            _ellipse.draw(this._color);
            shapes.push(_ellipse);
        } else if (this._isPolygon) {
            var _polygon: Polygon   = new Polygon(0, 0, this._points);
            _polygon.draw(this._color);
            shapes.push(_polygon);
        } else if (this._isPolyLine) {
            var _polyline: PolyLine = new PolyLine(0, 0, this._points);
            _polyline.draw(this._color);
            shapes.push(_polyline);
        } else {
            if (!this._gid) {
                var _polygon: Polygon = new Polygon(0, 0, [[0, 0], [this.width, 0], [this.width, this.height], [0, this.height]]);
                _polygon.draw(this._color);
                shapes.push(_polygon);
            }
        }
    ...
    private setTile(tilesets: TMXTilesetGroup): void {
        var tileset: TMXTileset = tilesets.getTilesetByGid(this._gid);
        if (tileset) {
            var image: TMXImage = tileset.image;
            if (image) {
