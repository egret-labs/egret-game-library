            ...
            if ($data) {
                ...
                if (children) {
                    for (var i = 0; i < children.length; i++) {
                        ...
                        var frame = new tiled.TMXAnimationFrame(child, x, y, tilemap, tileset);
...
(function (tiled) {
    ...
    tiled.TexturePool = TexturePool;
    egret.registerClass(TexturePool,"tiled.TexturePool");
})(tiled || (tiled = {}));
    ...
    var TMXImageLayer = (function (_super) {
        ...
        p.loadImage = function ($url) {
            ...
            tiled.BitmapLoader.load($url, function ($url) {
                var _texture = tiled.TexturePool.getTexture($url);
...
(function (tiled) {
    var BitmapLoader = (function () {
        ...
        BitmapLoader.onRESComplete = function (textureData) {
            var This = this;
            tiled.TexturePool.addTexture(This.url, textureData);
            //完成回调
    ...
    tiled.BitmapLoader = BitmapLoader;
    egret.registerClass(BitmapLoader,"tiled.BitmapLoader");
})(tiled || (tiled = {}));
...
(function (tiled) {
    var TMXLayer = (function (_super) {
                ...
                for (var i = 0; i < children.length; i++) {
                    ...
                    switch (child.localName) {
                        case tiled.TMXConstants.DATA:
                            //解析数据
                            this.setLayerData(tiled.TMXUtils.decode(child, child.attributes.encoding, child.attributes.compression));
                            break;
                        case tiled.TMXConstants.PROPERTIES:
                            //解析属性
        ...
        p.setTile = function (x, y, tileId) {
            ...
            if (this.tileset) {
                var tile = this.layerData[x][y] = new tiled.TMXTile(x, y, tileId, this.tilemap, this.tileset);
                return tile;
            }
            return null;
        };
...
(function (tiled) {
    var TMXObjectGroup = (function (_super) {
        ...
        p.showObjectById = function (id) {
            var object = this.getObjectById(id);
            if (object)
                object.visible = true;
        };
        p.hideObjectById = function (id) {
            var object = this.getObjectById(id);
            if (object)
